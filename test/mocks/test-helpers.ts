...
/**
 * Assert that logger was called a specific number of times
 *
 * Useful for verifying expected logging behavior.
 *
 * @param logger - Semantic-release logger instance
 * @param method - Logger method to check ('log', 'error', or 'success')
 * @param count - Expected number of calls
 *
 * @example
 * ```typescript
 * expectLogCallCount(context.logger, 'log', 3);
 * expectLogCallCount(context.logger, 'error', 0);
 * ```
 */
export function expectLogCallCount(
  logger: SemanticReleaseContext["logger"],
  method: "error" | "log" | "success",
  count: number,
): void {
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
  const calls = (logger[method] as Mock).mock.calls;
...
/**
 * Get all logged messages as an array of strings
 *
 * Useful for complex log assertions or debugging test failures.
 *
 * @param logger - Semantic-release logger instance
 * @param method - Logger method to extract ('log', 'error', or 'success')
 * @returns Array of log messages
 *
 * @example
 * ```typescript
 * const logs = getLogMessages(context.logger, 'log');
 * expect(logs).toContain('Starting analysis');
...
export function getLogMessages(
  ...
  return calls
    .flat()
    .filter((argument: unknown) => typeof argument === "string");
}
