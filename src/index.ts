/**
 * Semantic Release Jira Plugin - Class-Based Implementation
 *
 * This file contains the refactored class-based implementation that removes
 * module-level state. Once ready, this should replace src/index.ts
 *
 * Key changes:
 * - Module-level state moved to class instance properties
 * - Exported functions converted to class methods
 * - Added type guard for initialization checking
 * - Extracted large functions into smaller private methods
...
export class SemanticReleaseJiraPlugin {
  ...
  async analyzeCommits(
    ...
    const allIssues = parseIssueKeys(collected.issues);
